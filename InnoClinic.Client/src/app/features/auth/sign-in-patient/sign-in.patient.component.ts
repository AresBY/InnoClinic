...
export class SignInPatientComponent implements OnInit {
  ...
  constructor(
    ...
    private dialogRef: MatDialogRef<SignInPatientComponent>,
  ...
  onSubmit() {
    ...
    this.authService.signIn$(this.signInForm.value, false).subscribe({
      next: (response: any) => {
  ...
  openRegisterModal(event: Event) {
    ...
    this.dialog.open(RegisterPatientComponent, {
      width: '600px'
    });
  }
